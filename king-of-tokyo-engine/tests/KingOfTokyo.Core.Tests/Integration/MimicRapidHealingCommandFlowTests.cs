...
public sealed class MimicRapidHealingCommandFlowTests
{
    [Fact]
    public void ActivateRapidHealing_Should_Work_WhenPlayerMimicsRapidHealing()
    {
    ...
    [Fact]
    public void ActivateRapidHealing_Should_Fail_WhenMimicTargetsDifferentCard()
    {
        var gameState = CreateGameState(4);
        var player = gameState.GetCurrentPlayer();
        ...
        Assert.Equal("Player cannot use Rapid Healing right now.", result.Error);
