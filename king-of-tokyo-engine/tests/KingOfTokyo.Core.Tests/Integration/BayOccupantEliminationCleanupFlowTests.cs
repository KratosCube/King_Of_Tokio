...
public sealed class BayOccupantEliminationCleanupFlowTests
{
    ...
    [Fact]
    public void ChooseLeaveTokyo_Should_MoveAttackerIntoCity_WhenBayWasDisabledAfterBayOccupantElimination()
    {
        var gameState = CreateGameState(5);
        var attacker = gameState.GetCurrentPlayer();
        var cityOccupant = gameState.GetPlayerById(1);
        ...
        Assert.Equal(cityOccupant.PlayerId, finalizeResult.PendingDecision!.PlayerId);
