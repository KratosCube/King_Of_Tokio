...
public sealed class BayEnterWhenCityDefenderStaysFlowTests
{
    [Fact]
    public void ChooseLeaveTokyo_Should_MoveAttackerIntoBay_WhenCityDefenderStaysAndBayIsEmpty()
    {
        ...
        Assert.Contains(stayResult.NewEvents, e => e is TokyoEnteredEvent entered &&
                                                entered.PlayerId == attacker.PlayerId &&
                                                entered.Slot == TokyoSlot.Bay);
        Assert.Contains(stayResult.NewEvents, e => e is VictoryPointsGainedEvent gained &&
