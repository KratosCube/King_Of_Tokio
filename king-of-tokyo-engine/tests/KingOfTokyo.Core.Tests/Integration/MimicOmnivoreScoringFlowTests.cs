...
public sealed class MimicOmnivoreScoringFlowTests
{
    [Fact]
    public void FinalizeDice_Should_GainOmnivoreBonus_WhenPlayerMimicsOmnivore()
    {
        var gameState = CreateGameState(4);
        var player = gameState.GetCurrentPlayer();
        var copiedCardOwner = gameState.GetPlayerById(1);
        copiedCardOwner.AddKeepCard(CreateKeepCard(KnownCardIds.Omnivore, "Omnivore", 4));
        player.AddKeepCard(CreateMimicCopying(copiedCardOwner.PlayerId, KnownCardIds.Omnivore, "Omnivore"));
        ...
        Assert.False(player.HasKeepCard(KnownCardIds.Omnivore));
